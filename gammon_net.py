...
class Gammon_Net(nn.Module):

    def __init__(self, input_dim=198, hidden1_dim=396, hidden2_dim=80,
                 output_dim=4, alpha=0.1, lambda_param=0.7, device='cpu'):
        '''
        ...
        '''
        super(Gammon_Net, self).__init__()
        self.w1 = nn.Linear(input_dim, hidden1_dim)
        self.w1.weight = nn.Parameter(torch.zeros(hidden1_dim, input_dim))
        self.w1.bias = nn.Parameter(torch.zeros(hidden1_dim))
        self.w2 = nn.Linear(hidden1_dim, hidden2_dim)
        self.w2.weight = nn.Parameter(torch.zeros(hidden2_dim, hidden1_dim))
        self.w2.bias = nn.Parameter(torch.zeros(hidden2_dim))
        self.w3 = nn.Linear(hidden2_dim, output_dim)
        self.w3.weight = nn.Parameter(torch.zeros(output_dim, hidden2_dim))
        self.w3.bias = nn.Parameter(torch.zeros(output_dim))
        self.to(device)
        self.device = device
...
class Gammon_Net_Wrapper():
    def __init__(self, input_dim=198, hidden1_dim=396, hidden2_dim=80, output_dim=4, alpha=0.1,
                 lambda_param=0.7, device='cpu'):

        self.net = Gammon_Net(input_dim=input_dim, hidden1_dim=hidden1_dim,
                              hidden2_dim=hidden2_dim, output_dim=output_dim,
                              alpha=alpha, lambda_param=lambda_param, device=device)
